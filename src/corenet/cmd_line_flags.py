...
def flag(help_message: str, *, default: Any = None,
         arg_type: ArgType = ArgType.FLAG, short_name: Optional[str] = None):
  ...
  return dataclasses.field(
      default=default,
      metadata={"help": help_message, "arg_type": arg_type,
                "short_name": short_name})


def parse_flags(flag_struct_type: Type[T],
                flags: Optional[Sequence[str]] = None) -> T:
  ...
  for field in dataclasses.fields(flag_struct_type):
    ...
    if flag_type in {str, int, float}:
      ...
      parser.add_argument(*flag_name, type=flag_type,
                          default=default_value, help=help_message,
                          **kwargs)
    elif field.type == bool:
      assert not is_list
      group = parser.add_mutually_exclusive_group(required=is_required)
      group.add_argument(*flag_name, default=default_value,
                         dest=field.name, action="store_true",
                         help=help_message)
      neg_flag_name = [re.sub(r"^(--?)", r"\1no", v) for v in flag_name]
      group.add_argument(*neg_flag_name, default=default_value,
  ...
  result_args = {k: (v if v != list_flag_marker else list())
