...
layout(binding=2) buffer texcoords { float texcoord_buffer[]; };

// The material ids, 1 integer per triangle. Each value in this buffer
// is an index into the "materials" buffer.
layout(binding=3) buffer material_ids { int material_id_buffer[]; };

// The materials used in the scene.
layout(std430, binding=4) buffer materials { Material material_buffer[]; };
