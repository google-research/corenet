...
class EGL:
  ...
  def __init__(self, egl_lib_path):
    ...
    self.eglGetError = ctypes.CFUNCTYPE(
        EGL.EGLint
    ...
    self.eglBindAPI = ctypes.CFUNCTYPE(
        EGL.EGLBoolean, EGL.EGLenum
...
class EglContext:
  _context_cache = {}  # type: Dict[(int, Text), (EGL, EGL.EGLDisplay)]

  def __init__(self, cuda_device: int, egl_lib_path: Text):
    key = (cuda_device, egl_lib_path)
    if key in EglContext._context_cache:
      log.debug(f"Using cached EGL context for cuda device {cuda_device}")
      self._egl, self._display = EglContext._context_cache[key]
    else:
      log.debug(f"Creating EGL context for cuda device {cuda_device}")
      self._egl = EGL(egl_lib_path)
      self._display = self._create_display(cuda_device)
      EglContext._context_cache[key] = (self._egl, self._display)
    self._context = self._create_context(self._display)
    self.__enter__()

  def _egl_check_success(self, pred: bool, msg: Text, level=logging.FATAL):
  ...
  def _get_egl_devices(self):
    max_devices = 64
    devices = (EGL.EGLDeviceEXT * max_devices)()
    num_devices = EGL.EGLint()
    self._egl_check_success(
        self._egl.eglQueryDevicesEXT(max_devices, devices,
                                     ctypes.pointer(num_devices)),
        "Failed to retrieve EGL devices")
    log.debug(f"Found {num_devices} GPUs with EGL support.")

    result = {}  # type: Dict[int, EGL.EGLDeviceEXT]
    for device_idx, device in enumerate(list(devices[:num_devices.value])):
      device_extensions = (
          self._egl.eglQueryDeviceStringEXT(device,
                                            EGL.EGL_EXTENSIONS))  # type: bytes
      self._egl_check_success(bool(device_extensions),
                              "Unable to retrieve device extensions")
      ...
      st = self._egl.eglQueryDeviceAttribEXT(device, EGL.EGL_CUDA_DEVICE_NV,
                                             ctypes.pointer(cuda_device_attr))
      if not st or self._egl.eglGetError() != EGL.EGL_SUCCESS:
  ...
  def _create_display(self, cuda_device: int):
    ...
    self._egl_check_success(self._egl.eglInitialize(
        display, ctypes.pointer(major), ctypes.pointer(minor)),
        "Unable to initialize display")

    self._egl_check_success(self._egl.eglBindAPI(EGL.EGL_OPENGL_API),
  ...
  def _create_context(self, display: EGL.EGLDisplay):
    ...
    ct_config_attrib_list = (
        EGL.EGLint * len(config_attrib_list))(*config_attrib_list)

    config = EGL.EGLConfig()
    num_config = EGL.EGLint()
    is_successful = self._egl.eglChooseConfig(display, ct_config_attrib_list,
    ...
    context = self._egl.eglCreateContext(display, config, None,
  ...
  def load(self, name: Text) -> int:
    ...
    return self._egl.load_function(name.encode())
  ...
  def __enter__(self):
    ...
    self._egl_check_success(
        self._egl.eglMakeCurrent(self._display, EGL.EGL_NO_SURFACE,
...
def create_moderngl_context(*args, **kwargs):
  assert len(args) == 0
  egl_lib_path = os.environ.get("GLCONTEXT_LIBEGL",
                                kwargs.get("libegl", "libEGL.so.1"))
  if "cuda_device" not in kwargs:
    raise ContextError("cuda_device must be specified.")
  cuda_device = kwargs["cuda_device"]
  return EglContext(cuda_device, egl_lib_path)
