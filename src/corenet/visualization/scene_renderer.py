...
from corenet.gl import rasterizer
...
def load_textures(
  ...
  for encoded_image in encoded_images:
    if encoded_image not in image_to_index:
      ...
      image = np.array(
          pil_image
            .convert("RGB")
            .resize((width, height), resample=PIL.Image.BICUBIC))
  ...
  image_indices = util.to_tensor(image_indices, t.int32, "cpu")
  if texture_array:
    texture_array = util.to_tensor(texture_array, t.uint8, "cpu")
  else:
...
def render_scene(
  ...
  assert (len(vertex_positions.shape) == 3 and
          vertex_positions.shape[1:] == (3, 3))
  ...
  if view_projection_matrix is None:
    view_projection_matrix = camera_util.get_default_camera_for_mesh(
        vertex_positions)
  view_projection_matrix = util.to_tensor(view_projection_matrix,
                                          t.float32, "cpu")
  ...
  def create_coefficient_array(cur_tensor: InputTensor, num_channels,
    ...
    arr = cur_tensor
    if arr is None:
      arr = (t.ones([num_used_materials, num_channels], dtype=t.float32) *
  ...
  diffuse_texture_indices = util.to_tensor(diffuse_texture_indices,
                                           t.int32, "cpu")
  ...
  assert (diffuse_textures.shape[0] == num_used_textures and
          diffuse_textures.shape[3] == 3)
  ...
  if debug_io_buffer is not None:
    render_args.append(
        rasterizer.Buffer(5, debug_io_buffer, is_io=True))
